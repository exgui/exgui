...
use exgui::{builder::*, Model, ChangeView, Node, Comp, Color, Stroke, LineJoin, PathCommand::*};
...
impl Smile {
    fn view_mouth(&self) -> Node<Self> {
        if self.normal_face {
            path(vec![Move([100.0, 180.0]), BezCtrl([150.0, 230.0]), QuadBezTo([200.0, 180.0]), BezCtrl([150.0, 210.0]), QuadBezTo([100.0, 180.0])])
                .stroke(Stroke { width: 5.0, line_join: LineJoin::Round, ..Default::default() })
                .build()
        } else {
            path(vec![Move([100.0, 205.0]), BezCtrl([150.0, 155.0]), QuadBezTo([200.0, 205.0]), BezCtrl([150.0, 175.0]), QuadBezTo([100.0, 205.0])])
                .stroke(Stroke { width: 5.0, line_join: LineJoin::Round, ..Default::default() })
                .build()
        }
    }
